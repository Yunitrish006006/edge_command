...
TfLiteStatus RunNoFeatureGoldenTest() {
  ...
  TfLiteStatus status = GenerateFeatures(
...
TfLiteStatus RunYesFeatureGoldenTest() {
  micro_test::did_test_fail = false;
...
TfLiteStatus RunInferenceGoldenTest(const char* label,
                                    const int16_t* audio_data,
                                    size_t audio_data_size) {
  micro_test::did_test_fail = false;
  MicroPrintf("Running golden inference for <%s>", label);
  return TestAudioSample(label, audio_data, audio_data_size);
}

}  // namespace micro_speech_golden

TfLiteStatus RunMicroSpeechGoldenTests() {
  using micro_speech_golden::RunInferenceGoldenTest;
  using micro_speech_golden::RunNoFeatureGoldenTest;
  using micro_speech_golden::RunYesFeatureGoldenTest;

  MicroPrintf("Starting Micro Speech golden tests");

  TfLiteStatus status = RunNoFeatureGoldenTest();
  if (status != kTfLiteOk) {
    MicroPrintf("NoFeatureTest FAILED");
    return status;
  }

  status = RunYesFeatureGoldenTest();
  if (status != kTfLiteOk) {
    MicroPrintf("YesFeatureTest FAILED");
    return status;
  }

  status = RunInferenceGoldenTest("no", g_no_1000ms_audio_data,
                                  g_no_1000ms_audio_data_size);
  if (status != kTfLiteOk) {
    MicroPrintf("NoTest FAILED");
    return status;
  }

  status = RunInferenceGoldenTest("yes", g_yes_1000ms_audio_data,
                                  g_yes_1000ms_audio_data_size);
  if (status != kTfLiteOk) {
    MicroPrintf("YesTest FAILED");
    return status;
  }

  status = RunInferenceGoldenTest("silence", g_silence_1000ms_audio_data,
                                  g_silence_1000ms_audio_data_size);
  if (status != kTfLiteOk) {
    MicroPrintf("SilenceTest FAILED");
    return status;
  }

  status = RunInferenceGoldenTest("silence", g_noise_1000ms_audio_data,
                                  g_noise_1000ms_audio_data_size);
  if (status != kTfLiteOk) {
    MicroPrintf("NoiseTest FAILED");
    return status;
  }
...
bool RunKwsGoldenTests() { return RunMicroSpeechGoldenTests() == kTfLiteOk; }
...
TF_LITE_MICRO_TEST(NoFeatureTest) {
  TF_LITE_ENSURE_STATUS(micro_speech_golden::RunNoFeatureGoldenTest());
}

TF_LITE_MICRO_TEST(YesFeatureTest) {
  TF_LITE_ENSURE_STATUS(micro_speech_golden::RunYesFeatureGoldenTest());
}

TF_LITE_MICRO_TEST(NoTest) {
  TF_LITE_ENSURE_STATUS(micro_speech_golden::RunInferenceGoldenTest(
      "no", g_no_1000ms_audio_data, g_no_1000ms_audio_data_size));
}

TF_LITE_MICRO_TEST(YesTest) {
  TF_LITE_ENSURE_STATUS(micro_speech_golden::RunInferenceGoldenTest(
      "yes", g_yes_1000ms_audio_data, g_yes_1000ms_audio_data_size));
}

TF_LITE_MICRO_TEST(SilenceTest) {
  TF_LITE_ENSURE_STATUS(micro_speech_golden::RunInferenceGoldenTest(
      "silence", g_silence_1000ms_audio_data, g_silence_1000ms_audio_data_size));
}

TF_LITE_MICRO_TEST(NoiseTest) {
  TF_LITE_ENSURE_STATUS(micro_speech_golden::RunInferenceGoldenTest(
      "silence", g_noise_1000ms_audio_data, g_noise_1000ms_audio_data_size));
}
